...
class PredictionModel:
    def __init__(
        ...
        self.support_synergy = support_synergy
    ...
    def time_strength(self, team: list, duration: float, duration_ind: int = 0) -> float:
        cat = duration_ind if duration_ind > 0 else _duration_to_category(duration)
        values = np.array(
            [
    ...
    def predict(self, radiant: list, dire: list) -> list[dict]:
        ...
        for duration_cat in range(1,9):
            ...
            with torch.no_grad():
                ...
                p_rad = torch.sigmoid(pred).item()
            results.append({"Radiant": p_rad, "Dire": 1 - p_rad, "Time": duration_cat})
